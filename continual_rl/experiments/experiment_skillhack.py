import os
import json
from continual_rl.experiments.run_metadata import RunMetadataSkills 
...
class ExperimentSkills(object):
    def __init__(
        ...
        """
        The Experiment class contains everything that 
        should be held consistent when the experiment is used as a
        setting for a baseline.

        A single experiment can cover tasks with a variety of action spaces. 
        It is up to the policy on how they wish to handle this, 
        but what the Experiment does is create a dictionary 
        mapping action_space_id to action space, and
        ensures that all tasks claiming the same id use the same action space.

        The observation space and time batch sizes are both restricted 
        to being the same for all tasks. This
        initialization will assert if this is violated.

        :param skills: A list of subclass of TaskBase. These need to 
            have a consistent observation space.
        :param tasks: A list of subclasses of TaskBase. These need to 
            have a consistent observation space.
        :param output_dir: The directory in which logs will be stored.
        :param continual_testing_freq: The number of timesteps between evaluation
             steps on the not-currently-training
        tasks.
    ...
    def _run(self, policy, summary_writer):
        ...
        run_metadata = RunMetadataSkills(self._output_dir)
        start_cycle_id = run_metadata.cycle_id
        start_skill_id = run_metadata.skill_id
        start_skill_timesteps = run_metadata.skill_timesteps
        ...
        for cycle_id in range(start_cycle_id, self._cycle_count):
            for skill_run_id, skill in enumerate(
                self.skill[start_skill_id:], 
                start=start_skill_id,
            ):
                # Run the current task as a generator so we can intersperse testing tasks during the run
                self._logger.info(f"Starting cycle {cycle_id} skill {skill_run_id}")
                skill_complete = False
                skill_runner = skill.run(
                    skill_run_id,
                    policy,
                    summary_writer,
                    self.output_dir,
                    timestep_log_offset=total_train_timesteps,
                    task_timestep_start=start_skill_timesteps,
                )
                skill_timesteps = start_skill_timesteps  # What timestep the task is currently on. Cumulative during a task.
                ...
                while not skill_complete:
                    try:
                        skill_timesteps, _ = next(skill_runner)
                    except StopIteration:
                        skill_complete = True

                    if not skill._task_spec.eval_mode:
                        if steps_since_save >= save_every_steps or skill_complete:
                            ...
                            if skill_complete:
                                task_boundary_dir = os.path.join(
                                    self.output_dir,
                                    f'cycle{cycle_id}_task{skill_run_id}',
                                )
                    ...
                    if continual_freq is not None and not skill._task_spec.eval_mode and \
                            total_train_timesteps + skill_timesteps > last_continual_testing_step + continual_freq:
                        self._run_continual_eval(
                            skill_run_id,
                            policy,
                            summary_writer,
                            total_train_timesteps + skill_timesteps,
                        )
                        last_continual_testing_step = total_train_timesteps + skill_timesteps

                # Log out some info about the just-completed task
                self._logger.info(f"Skill {skill_run_id} complete")

                # Train on tasks
                # TODO
                for task_run_id, task in enumerate(self.tasks):
                    # log tasks as a new metric on tensorboard
                    task_run_id += len(self.skills)
                    start_task_timesteps = 0
                    last_task_continual_testing_step = last_continual_testing_step

                    self._logger.info(f"Starting training task {task_run_id}")
                    if task_run_id > 0:
                        policy.load(self.output_dir)
                    task_complete = False
                    ...
                    while not task_complete:
                        ...
                        # We don't save the policy so that we can load it up 
                        # again for the next task

                        # If we're already doing eval, don't do a forced eval run (nothing has trained to warrant it anyway)
                        # Evaluate intermittently. Every time is too slow
                        if continual_freq is not None and not task._task_spec.eval_mode and \
                                total_train_timesteps + task_timesteps > last_task_continual_testing_step + continual_freq:
                            self._run_continual_eval(
                                task_run_id,
                                policy,
                                summary_writer,
                            ...
                            # total_train_timesteps is not updated in the task loop
                            last_task_continual_testing_step = total_train_timesteps + task_timesteps

                    # Log out some info about the just-completed task
                    self._logger.info(f"Task {task_run_id} complete")    

                # Re-load up original policy before we trained on the tasks
                policy.load(self.output_dir)
